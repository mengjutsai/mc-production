...
generate p p > t t~ h3, (h3 > t t~, (t > b w+, w+ > wdec wdec), (t~ > b~ w-, w- > wdec wdec)), (t > b w+, w+ > wdec wdec), (t~ > b~ w-, w- > wdec wdec)
output -f
"""
...
for s in model_pars_str.split("_"):
    ...
    if 'mh3' in s:
        ...
        if ss.isdigit():
            mh3 = int(ss)
            print  "BSM Higgs mass mh3 set to %i"%mh3
    if 'wh3' in s:
        ss=s.replace("wh3","")
        if ss.isdigit():
            wh3 = int(ss)
            print  "BSM Higgs mass wh3 set to %i"%wh3


seed_num = randrange(500000)
print "metsai - random seed %i" % seed_num
...
masses={'35':str(mh3)+' # mh3',
        '36':str(mh3),
        '37':str(mh3),
        '25':str(125),

        }
# parameters={'35':str(wh3)}
parameters={'35':
str(wh3)+"""
#  BR             NDA  ID1    ID2   ...
